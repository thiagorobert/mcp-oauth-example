...
# Create handler if one doesn't exist
...
# Ensure this logger's level is always DEBUG regardless of root logger
logger.setLevel(logging.DEBUG)
...
async def authenticate() -> str | None:
    ...
    logger.debug(f" 1. Go to: {device_info['verification_uri']}")
    logger.debug(f" 2. Enter code: {device_info['user_code']}")
    logger.debug(f" 3. Waiting for authentication...")
    logger.debug(f" (Code expires in {device_info.get('expires_in', 900)} seconds)")
...
async def list_repositories() -> str:
    ...
    logger.debug(f"GITHUB_CLIENT_ID set: {bool(GITHUB_CLIENT_ID)}")
    logger.debug(f"GITHUB_CLIENT_SECRET set: {bool(GITHUB_CLIENT_SECRET)}")
